...
export default function PricingCard({ plan, index = 0 }: { plan: PricingPlan; index?: number }) {
  return (
    ...
    >
      {plan.popular && (
        <span className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 text-xs font-semibold rounded-full gradient-bg text-primary-foreground">Most Popular</span>
      )}
      ...
      <button className={`w-full py-3 rounded-lg font-medium text-sm transition-all ${plan.popular ? "gradient-bg text-primary-foreground hover:opacity-90" : "border border-border hover:bg-muted/50"}`}>
        {plan.cta}
      </button>
    </motion.div>
  );
