...
import { Star, Check, ArrowLeft, Download, ShoppingCart } from "lucide-react";
        ...
        <div className="grid lg:grid-cols-5 gap-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[product.image, product.image, product.image].map((img, i) => (
                <div key={i} className="rounded-xl overflow-hidden border border-border cursor-pointer hover:border-primary/50 transition-colors">
            ...
            {/* Description */}
            <div className="mt-10">
              <h2 className="font-heading text-xl font-bold mb-4">About this product</h2>
            ...
            {/* Reviews */}
            <div className="mt-10">
              <h2 className="font-heading text-xl font-bold mb-4">Reviews ({product.reviews})</h2>
              {[1, 2, 3].map(i => (
                <div key={i} className="flex gap-4 mb-5 pb-5 border-b border-border/50 last:border-0">
                  <img src={`https://images.unsplash.com/photo-${i === 1 ? '1494790108377-be9c29b29330' : i === 2 ? '1507003211169-0a1dd7228f2d' : '1438761681033-6461ffad8d80'}?w=40&h=40&fit=crop&crop=face`} alt="" className="w-10 h-10 rounded-full object-cover" />
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-semibold">User {i}</span>
                      <div className="flex">{Array.from({ length: 5 }).map((_, j) => <Star key={j} className="w-3 h-3 fill-warning text-warning" />)}</div>
                    </div>
                    <p className="text-sm text-muted-foreground">Excellent product! Highly recommended for any development team looking to streamline their workflow.</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="lg:col-span-2">
            <div className="sticky top-24 p-6 rounded-2xl border border-border bg-card">
              <span className="text-xs font-medium text-primary mb-2 block">{product.category}</span>
              <h1 className="font-heading text-2xl font-bold mb-2">{product.name}</h1>
              <div className="flex items-center gap-2 mb-4">
                <Star className="w-4 h-4 fill-warning text-warning" />
                <span className="text-sm font-medium">{product.rating}</span>
                <span className="text-sm text-muted-foreground">({product.reviews} reviews)</span>
                {product.downloads && <span className="text-sm text-muted-foreground ml-auto">{product.downloads.toLocaleString()} downloads</span>}
              </div>
              <div className="flex items-end gap-2 mb-6">
                ...
                {product.originalPrice && <span className="text-sm font-medium text-success ml-1">Save {Math.round((1 - product.price / product.originalPrice) * 100)}%</span>}
              </div>
              ...
              <div className="mt-6 pt-6 border-t border-border/50 space-y-3">
                {["Instant download", "Lifetime access", "Free updates", "Priority support"].map(f => (
